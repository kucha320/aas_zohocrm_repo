...
class zcrmapi():
    ...
    def get_api_users(self, user_type):
        try:
            imports = []
    ...
    def api_get_leads(self, allrecords):
        try:
    ...
    def api_get_contacts(self, allrecords):
        try:
    ...
    def denormalize_multiselect(self, EntityType, SourceType, mylists):
        outputs = []
        d = {}
        #SourceType = 'Sources'
    ...
    def api_get_institutions(self, allrecords):
        try:
    ...
    def api_get_programs(self, allrecords):
        try:
    ...
    def api_get_applications(self, allrecords):
        try:
            ...
            while LoopAgain:
                ...
                for record_ins in record_ins_arr:
                    ...
                    ### Get App Program ###
                    d2 = {}
                    d2 = self.api_get_application_programs(d['Entity ID'])
                    if d2 is not None :
                        programs.append(d2.copy())  
                    
                    ### Get Stage History ###
                    d3 = {}
                    d3 = self.api_get_application_stage_history(d['Entity ID'])
    ...
    def api_get_application_programs(self, app_id):
        try:
            imports = []
            d = {}
    ...
    def api_get_application_stage_history(self, app_id):
        try:
            imports = []
            d = {}
            record = zcrmsdk.ZCRMRecord.get_instance('Deals', app_id)  # module API Name, entityId
    ...
    def api_get_campaigns(self, allrecords):
        try:
            per_page = 200 #max for crm api
            page = 0#1
    ...
    def api_get_tasks(self, allrecords, forchecklist):
        try:
            per_page = 200 #max for crm api
            page = 0#1
    ...
    def api_get_deleted_records(self, module, deleted_since):
        try:
            per_page = 200 #max for crm api
            page = 0#1
            imports = []
    ...
    def api_update_app_checklist(self, mylists):
        try:
            count = 0
            ...
            for mylist in mylists:
                ...
                if shortcode == 'VA':
                    record.set_field_value('Visa_Submission', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'VD':
                    record.set_field_value('Visa_Done', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'AA':
                    record.set_field_value('Accommodation_Application', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'AD':
                    record.set_field_value('Accommodation_Done', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'TA':
                    record.set_field_value('Ticket_Quoted', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'TD':
                    record.set_field_value('Ticket_Done', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'PA':
                    record.set_field_value('Pickup_Request', self.api_datetime_to_date(mylist['Closed Date']))
                if shortcode == 'PD':
                    record.set_field_value('Pickup_Done', self.api_datetime_to_date(mylist['Closed Date']))
                resp=record.update()
